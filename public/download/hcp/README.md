...
```bash
curl -fsSL https://wor.worapong.com/download/hcp/installer-hcp.sh | bash -s -- v0.1.0
```
...
```bash
curl -fsSL https://wor.worapong.com/download/hcp/installer-hcp.sh | bash -s -- v0.1.0 --target=example/admin-panel --dry-run
```
